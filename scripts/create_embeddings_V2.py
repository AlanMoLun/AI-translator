...
# 动态获取 embedding 维度
try:
    test_resp = client.embeddings.create(model=clientConfig["model"], input="测试")
    embedding_dim = len(test_resp.data[0].embedding)
except Exception as e:
    print(f"⚠️ 无法获取 embedding 维度，使用默认 1536: {e}")
    embedding_dim = 1536

for idx, row in df.iterrows():
    zh_text = str(row["zh"]).strip()
    if not zh_text:
        print(f"⚠️ 第 {idx} 行 zh 为空，跳过")
        continue

    # 决定 selected_text（用于翻译）
    selected_text = ""
    if pd.notna(row["en"]) and str(row["en"]).strip():
        selected_text = str(row["en"]).strip()
    elif pd.notna(row["en_v"]) and str(row["en_v"]).strip():
        selected_text = str(row["en_v"]).strip()
    elif pd.notna(row["pali_sanskrit"]) and str(row["pali_sanskrit"]).strip():
        selected_text = str(row["pali_sanskrit"]).strip()
    else:
        selected_text = zh_text

    try:
        resp = client.embeddings.create(
            model=clientConfig["model"],
            input=zh_text  # ← 关键：用中文！
        )
        emb = resp.data[0].embedding
        embeddings.append(emb)
        selected_texts.append(selected_text)
        source_columns.append("zh")  # embedding 来源
    except Exception as e:
        print(f"❌ 生成 '{zh_text}' 的 embedding 失败: {e}")
        # 可选择跳过，或填零向量（不推荐）
        continue
